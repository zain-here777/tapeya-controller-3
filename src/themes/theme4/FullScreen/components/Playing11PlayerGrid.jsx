    ...
    >
      {Array.from({ length: ROW_COUNT }, (_, index) => {
        ...
        return (
          <div
            key={`${side}-row-${index}`}
            className={`${t4Motion("fadeUp")} ${rowTint} flex min-h-0 flex-1 items-center justify-start border-b ${P11_SECTION_DIVIDER} px-[calc(20px*var(--t4-scale))] last:border-b-0 max-[900px]:px-[calc(12px*var(--t4-scale))]`}
            style={t4StaggerStyle(index, staggerBase, 28)}
          >
            {name ? (
              <span className="min-w-0 truncate text-left text-[calc(26px*var(--t4-scale))] font-semibold uppercase leading-[1.15] tracking-[calc(0.6px*var(--t4-scale))] text-[#f8fafc] max-[900px]:text-[calc(20px*var(--t4-scale))]">
                {name}
              </span>
            ) : null}
          </div>
...
export default function Playing11PlayerGrid({
  ...
  return (
    <div
      className={`flex min-h-0 flex-1 ${P11_COLUMN_GAP} px-[calc(16px*var(--t4-scale))] pb-[calc(16px*var(--t4-scale))] pt-[calc(12px*var(--t4-scale))] max-[900px]:px-[calc(10px*var(--t4-scale))] max-[900px]:pb-[calc(10px*var(--t4-scale))] max-[900px]:pt-[calc(8px*var(--t4-scale))]`}
    >
