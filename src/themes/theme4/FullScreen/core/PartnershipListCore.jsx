  ...
  return (
    <FullScreenPageShell>
      <div className="mx-auto flex h-full w-full max-w-[calc(1480px*var(--t4-scale))] flex-col gap-[calc(28px*var(--t4-scale))] px-[calc(48px*var(--t4-scale))] py-[calc(36px*var(--t4-scale))]">
        <header
          className={`${t4Motion("fadeUp")} flex shrink-0 items-center gap-[calc(18px*var(--t4-scale))]`}
          style={t4StaggerStyle(0)}
        >
          <div className={`${T4_LOGO_TILE} h-[calc(72px*var(--t4-scale))] w-[calc(72px*var(--t4-scale))] p-[calc(6px*var(--t4-scale))]`}>
            {team.logoUrl ? (
              <img src={team.logoUrl} alt={teamLabel} className="h-full w-full object-contain" />
            ) : (
              <span className="text-[calc(18px*var(--t4-scale))] font-bold uppercase" style={{ color: teamAccent }}>
                {team.code || "?"}
              </span>
            )}
          </div>
          <div className="flex min-w-0 flex-col gap-[calc(2px*var(--t4-scale))]">
            {teamLabel ? (
        ...
        <div
          className={`${t4Motion("riseSoft")} relative my-auto flex h-[calc(620px*var(--t4-scale))] w-full flex-col gap-[calc(28px*var(--t4-scale))] rounded-[calc(12px*var(--t4-scale))] border border-[var(--t4-divider)] px-[calc(40px*var(--t4-scale))] py-[calc(36px*var(--t4-scale))] shadow-[0_0_calc(40px*var(--t4-scale))_rgba(0,0,0,0.35)]`}
          style={{ ...cardStyle, ...t4StaggerStyle(1, 100) }}
        >
          <div className="flex w-full flex-col gap-[calc(28px*var(--t4-scale))] overflow-hidden">
          ...
          <p className="shrink-0 text-center text-[calc(16px*var(--t4-scale))] font-semibold uppercase tracking-[calc(2px*var(--t4-scale))] text-[#f8fafc]/80">
            RUNS CONTRIBUTED
          </p>
        </div>
        ...
        {(summary.extras != null || summary.overs != null || summary.score) && (
          <div
            className="ml-auto flex shrink-0 items-center gap-[calc(18px*var(--t4-scale))] rounded-[calc(10px*var(--t4-scale))] border border-[var(--t4-divider)] px-[calc(22px*var(--t4-scale))] py-[calc(14px*var(--t4-scale))] shadow-[0_0_calc(24px*var(--t4-scale))_rgba(0,0,0,0.3)]"
            style={cardStyle}
          >
            {summary.extras != null ? (
              <span className="text-[calc(18px*var(--t4-scale))] font-semibold uppercase text-[#f8fafc]">
            ...
            {summary.score ? (
              <>
                <span className="w-px shrink-0 self-stretch bg-white/25" aria-hidden="true" />
                <span className="text-[calc(40px*var(--t4-scale))] font-black tabular-nums text-[#f8fafc]">
                  {summary.score}
                </span>
              </>
            ) : null}
