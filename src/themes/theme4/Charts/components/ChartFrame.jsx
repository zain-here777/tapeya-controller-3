...
export default function ChartFrame({
  ...
  const scale = "var(--t4-scale)";

  return (
    <div
      className="t4-chart-frame relative max-w-full"
      style={{
    ...
    >
      {yTicks.map((tick) => {
        ...
        return (
          <div key={tick}>
            <div
              className="t4-chart-grid-line absolute h-px"
              style={{
            ...
            />
            <span
              className="t4-chart-tick absolute text-right"
              style={{
                left: 0,
      ...
      {xLabels ? (
        ...
        >
          {xLabels.map((label, index) => (
            <span
              key={`${label}-${index}`}
              className="t4-chart-xlabel flex min-w-0 flex-1 justify-center text-center"
            >
              {label}
            </span>
          ))}
      ...
      {yLabel ? (
        <span
          className="t4-chart-axis-label absolute origin-left -rotate-90 uppercase"
          style={{
            left: `calc(-8px * ${scale})`,
      ...
      {xLabel ? (
        <span
          className="t4-chart-axis-label absolute text-center uppercase"
          style={{
            left: `calc(${PAD_L}px * ${scale})`,
            right: `calc(${PAD_R}px * ${scale})`,
            top: xLabels
