...
function PhaseBar({ value, yMax, plotH, color, delay, badge = null }) {
  ...
  return (
    <div
      className="t4-chart-phase-bar relative min-w-0 flex-1 origin-bottom motion-safe:animate-chart-bar-grow motion-reduce:animate-none"
      style={{
        height: `calc(${heightPx}px * var(--t4-scale))`,
        maxWidth: `calc(96px * var(--t4-scale))`,
        background: `linear-gradient(180deg, ${color}, color-mix(in srgb, ${color} 65%, #0b1220))`,
        animationDelay: `${delay}ms`,
      }}
    >
      {badge != null ? (
        <span className="t4-chart-wicket-badge">{badge}</span>
      ) : null}
    </div>
  );
}
  ...
  return (
    <div className="absolute inset-0 flex">
      {buckets.map((bucket, index) => (
        <div
          key={bucket.label}
          className="relative flex min-w-0 flex-1 items-end justify-center gap-[calc(14px*var(--t4-scale))] px-[calc(6px*var(--t4-scale))]"
        >
          <PhaseBar
            value={bucket.top}
            yMax={yMax}
...
function TeamStrip({ name, score, tone }) {
  return (
    <div
      className={`t4-chart-team-strip ${
        tone === "bowler" ? "t4-chart-team-strip--bowler" : ""
      }`}
    >
      <span className="t4-chart-strip-name uppercase">{name}</span>
      <span className="t4-chart-strip-score ml-auto tabular-nums">{score}</span>
    </div>
  );
}

/** Shared phase bar chart — Manhattan / Run Rate (Theme 4). */
export default function PhaseChartCore({
  ...
  return (
    <ChartPageShell>
      ...
      <div className="t4-chart-body t4-chart-body--phase t4-chart-body-anim">
        <div
          className="flex h-full min-w-0 w-full flex-col gap-[calc(28px*var(--t4-scale))]"
          style={{ maxWidth: `calc(${CHART_W}px * var(--t4-scale))` }}
        >
          ...
          <div className="flex w-full shrink-0 flex-col gap-[calc(12px*var(--t4-scale))]">
            <TeamStrip
