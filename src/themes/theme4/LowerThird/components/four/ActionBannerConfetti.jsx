...
const TONE_COLORS = {
  accent: "var(--t4-broadcast-accent, #0ea5e9)",
  sky: "var(--t4-broadcast-highlight, #7dd3fc)",
  white: "#f8fafc",
  gold: "#fbbf24",
};

/**
 * Theme 4 celebration confetti — rain behavior adapted into Theme 4 palette.
  ...
  return (
    ...
    >
      {CONFETTI_PIECES.map((piece, index) => {
        const sizeClass =
          index % 4 === 0
            ? "h-[calc(12px*var(--t4-scale))] w-[calc(5px*var(--t4-scale))]"
            : index % 3 === 0
              ? "h-[calc(10px*var(--t4-scale))] w-[calc(3px*var(--t4-scale))]"
              : "h-[calc(16px*var(--t4-scale))] w-[calc(8px*var(--t4-scale))]";
        ...
        return (
          <span
            ...
            className={`t4-action-confetti-piece absolute top-0 opacity-0 ${sizeClass}`}
            style={{
              left: piece.left,
              backgroundColor: TONE_COLORS[piece.tone],
              ["--t4-confetti-rotate"]: `${piece.rotate}deg`,
