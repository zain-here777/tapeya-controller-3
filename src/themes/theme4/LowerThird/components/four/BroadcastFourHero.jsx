...
/** Max staggered letter slots supported by Theme 4 broadcast CSS. */
...
export default function BroadcastFourHero({ label, textSizeClass }) {
  ...
  return (
    <div className="t4-broadcast-four-hero motion-reduce:hidden" aria-hidden="true">
      <span className={`t4-broadcast-four-word ${textSizeClass}`}>
        {letters.map((letter, index) => (
          <span
            key={`${letter}-${index}`}
            className={`t4-broadcast-four-letter t4-broadcast-four-letter--${index}`}
          >
            {letter === " " ? "\u00A0" : letter}
          </span>
        ))}
