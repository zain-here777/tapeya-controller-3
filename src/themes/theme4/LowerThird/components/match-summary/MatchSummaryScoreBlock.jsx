...
export default function MatchSummaryScoreBlock({
  ...
  return (
    <div className={`flex flex-col leading-none ${alignClass}`}>
      <span
        key={`${runs}-${wickets}`}
        className="animate-score-pop text-[calc(40px*var(--t4-scale))] font-bold tabular-nums text-white"
      >
        {runs}-{wickets}
      </span>
      {overs != null && overs !== "" ? (
        <span className="mt-[calc(6px*var(--t4-scale))] text-[calc(18px*var(--t4-scale))] font-medium uppercase tracking-wide text-white">
          {overs} OVER
        </span>
      ) : null}
    </div>
