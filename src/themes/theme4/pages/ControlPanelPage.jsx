...
function ControllerButtonRow({ controllers, active, onSelect }) {
  return (
    <div className="t4-panel-btn-row">
...
export default function Theme4ControlPanelPage() {
  ...
  return (
    <BroadcastPageShell className="t4-control-panel">
      <header className="t4-control-panel-header">
        <div>
          <p className="t4-control-panel-eyebrow">Scorecard Controller</p>
          <h1 className="t4-control-panel-title">Control Panel</h1>
        </div>
        <div className="t4-control-panel-actions">
          <ThemeSelector />
          <Link to={outputPath} target="_blank" rel="noreferrer" className="t4-panel-link-btn">
            Open output ↗
          </Link>
        </div>
      </header>
      ...
      <main className="t4-control-panel-main">
        {groups.map(({ category, label, controllers, groups: subGroups }) => {
          ...
          return (
            <section key={category} aria-labelledby={`t4-category-${category}`}>
              <h2 id={`t4-category-${category}`} className="t4-control-panel-section-title">
                {label}
              </h2>
