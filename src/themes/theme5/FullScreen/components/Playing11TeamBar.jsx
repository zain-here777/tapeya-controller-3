...
function TeamSide({ team, align }) {
  ...
  return (
    <div
      className={`flex min-w-0 flex-1 items-center gap-[calc(14px*var(--t5-scale))] px-[calc(28px*var(--t5-scale))] max-[900px]:px-[calc(16px*var(--t5-scale))] ${
        isRight ? "justify-end" : "justify-start"
      }`}
    >
      {!isRight ? (
        <div
          className={`${T4_LOGO_TILE} h-[calc(52px*var(--t5-scale))] w-[calc(52px*var(--t5-scale))] shrink-0 p-[calc(5px*var(--t5-scale))] max-[900px]:h-[calc(44px*var(--t5-scale))] max-[900px]:w-[calc(44px*var(--t5-scale))]`}
        >
          {team?.logoUrl ? (
            <img src={team.logoUrl} alt={label} className="h-full w-full object-contain" />
          ) : (
            <span className="text-[calc(14px*var(--t5-scale))] font-bold uppercase text-[#0f2847]">
              {team?.code || "?"}
            </span>
          )}
        </div>
      ...
      <span className="min-w-0 truncate text-[calc(32px*var(--t5-scale))] font-black uppercase leading-[1.05] tracking-[calc(1px*var(--t5-scale))] text-[#f8fafc] max-[900px]:text-[calc(24px*var(--t5-scale))]">
        {label}
      </span>
...
export default function Playing11TeamBar({ teamA, teamB, toneA = "batsman", toneB = "bowler" }) {
  ...
  return (
    <div
      className={`relative flex min-h-[calc(76px*var(--t5-scale))] shrink-0 items-stretch border-b ${P11_SECTION_DIVIDER} max-[900px]:min-h-[calc(64px*var(--t5-scale))]`}
    >
      ...
      <div
        ...
        aria-hidden="true"
      >
        <span className="text-[calc(22px*var(--t5-scale))] font-black uppercase leading-none text-[#070E35]">
          V
        </span>
      </div>
