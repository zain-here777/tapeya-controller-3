    ...
    <FullScreenPageShell>
      ...
      <div className="relative z-[1] mx-auto flex h-full w-full max-w-[calc(1480px*var(--t5-scale))] flex-col justify-center gap-[calc(20px*var(--t5-scale))] px-[calc(48px*var(--t5-scale))] py-[calc(36px*var(--t5-scale))]">
        ...
        <div className="flex items-center gap-[calc(48px*var(--t5-scale))]">
            ...
            <div className="flex w-full flex-col gap-[calc(8px*var(--t5-scale))]">
              {batters.map((batter, index) => {
                ...
                return (
                  <div
                    key={`${name}-${index}`}
                    className="grid w-full shrink-0 grid-cols-[minmax(0,1.15fr)_minmax(0,1fr)_auto] items-center gap-[calc(12px*var(--t5-scale))] rounded-[calc(8px*var(--t5-scale))] border border-white/15 px-[calc(18px*var(--t5-scale))] py-[calc(8px*var(--t5-scale))] motion-safe:animate-soft-in motion-reduce:animate-none"
                    style={{
                      ...fill,
                      minHeight: "calc(48px * var(--t5-scale))",
                      animationDelay: `${index * 60}ms`,
                    }}
                  >
                    <span className="truncate text-[calc(22px*var(--t5-scale))] font-bold uppercase leading-[1.1] tracking-[calc(0.8px*var(--t5-scale))] text-[#f8fafc]">
                      {name}
                      {strikeMark}
                    </span>
                    <span className="text-center text-[calc(16px*var(--t5-scale))] font-medium uppercase leading-[1.1] tracking-[calc(0.8px*var(--t5-scale))] text-[#f8fafc]">
                      {howOut}
                    </span>
                    <span className="flex min-w-[calc(72px*var(--t5-scale))] items-start justify-end gap-[calc(2px*var(--t5-scale))]">
                      <span className="text-[calc(26px*var(--t5-scale))] font-black tabular-nums leading-none tracking-[calc(-0.5px*var(--t5-scale))] text-[#f8fafc]">
                        {runs}
                      </span>
                      <span className="text-[calc(13px*var(--t5-scale))] font-semibold tabular-nums leading-none text-[#f8fafc]">
                        {balls}
                      </span>
                    </span>
                  </div>
          ...
          {/* bs-fs-crest — white tile like other Theme 5 logos */}
          <div
