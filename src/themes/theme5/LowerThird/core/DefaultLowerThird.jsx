...
import DefaultBowlingPanel from "../components/default/DefaultBowlingPanel.jsx";

function pickRunRateStat(match) {
  if (match.runRateStat) {
    return { ...match.runRateStat, label: "RR" };
  }
  if (!match.stat) return null;

  const label = match.stat.label || "";
  if (label === "CRR" || label === "RR" || /run rate/i.test(label)) {
    return {
      label: "RR",
      value: match.stat.value,
    };
  }
...
/**
 * Theme 5 Default broadcast lower third — batting | score | bowling with team color bars.
 *
 * @param {import('../../../../shared/types/match.types.js').MatchData} match
 * @param {Object} props.baseConfig
 * @param {Object} [props.config]
 */
export default function DefaultLowerThird({
  match,
  baseConfig,
