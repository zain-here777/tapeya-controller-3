...
function BatterRow({ batter }) {
  ...
  const textSize = onStrike
    ? "text-[calc(28px*var(--t5-scale))]"
    : "text-[calc(22px*var(--t5-scale))]";
  const ballsSize = onStrike
    ? "text-[calc(22px*var(--t5-scale))]"
    : "text-[calc(20px*var(--t5-scale))]";

  return (
    <div className="flex min-w-0 items-center gap-[calc(8px*var(--t5-scale))] leading-none">
      <span
        className={`inline-flex w-[calc(16px*var(--t5-scale))] shrink-0 items-center justify-center self-center${
          onStrike ? "" : " invisible"
        }`}
        aria-hidden="true"
      >
        {onStrike ? (
          <span className="block h-[calc(9px*var(--t5-scale))] w-[calc(9px*var(--t5-scale))] animate-strike-blink rounded-full bg-white shadow-[0_0_calc(6px*var(--t5-scale))_rgba(255,255,255,0.55)]" />
        ) : null}
      </span>

      <div className="flex min-w-0 items-center gap-[calc(24px*var(--t5-scale))]">
...
function OversRunRateAside({ overs, oversLimit, runRateStat, runRateStats }) {
  ...
  return (
    <div className="flex shrink-0 flex-col items-center justify-center gap-[calc(6px*var(--t5-scale))] leading-snug">
      {(overs || oversLimit != null) && (
        <span
          key={`${overs}-${oversLimit}`}
          className="animate-score-pop whitespace-nowrap text-center text-[calc(28px*var(--t5-scale))] font-medium tabular-nums"
        >
          {overs ?? "0"}
      ...
      {rates.length > 0 ? (
        <div className="flex flex-col items-center justify-center gap-[calc(4px*var(--t5-scale))]">
          {rates.map((stat) => (
            <span
              key={`${stat.label}-${stat.value}`}
              className="animate-soft-in whitespace-nowrap text-center text-[calc(22px*var(--t5-scale))] font-medium"
            >
              {stat.label}{" "}
...
function NeedTargetStatsAside({ stats }) {
  ...
  return (
    <div className="flex shrink-0 items-center gap-[calc(36px*var(--t5-scale))]">
      {stats.map((stat) => (
        <div
          key={stat.label}
          className="flex flex-col items-center justify-center gap-[calc(10px*var(--t5-scale))] leading-none"
        >
          <span className="animate-soft-in whitespace-nowrap text-center text-[calc(18px*var(--t5-scale))] font-bold uppercase tracking-wide text-white">
            {stat.label}
          </span>
          <span
            key={`${stat.label}-${stat.value}`}
            className="animate-score-pop whitespace-nowrap text-center text-[calc(28px*var(--t5-scale))] font-bold tabular-nums"
          >
            {stat.value}
          </span>
        </div>
...
export default function DefaultBattingPanel({
  ...
  return (
    <div
      className="flex min-w-0 flex-1 items-center gap-[calc(20px*var(--t5-scale))] px-[calc(18px*var(--t5-scale))] text-white"
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-center gap-[calc(21px*var(--t5-scale))]">
