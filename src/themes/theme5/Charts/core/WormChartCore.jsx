...
function MiniStat({ label, value }) {
  return (
    <div className="t5-chart-mini-stat flex-1">
      <span className="t5-chart-mini-label uppercase">{label}</span>
      <span className="t5-chart-mini-value tabular-nums">{value}</span>
    </div>
  );
}
...
function WormMeta({ name, meta, tone }) {
  return (
    <div className="flex min-w-0 flex-1 flex-col gap-[calc(12px*var(--t5-scale))]">
      <div className="t5-chart-meta-row">
        <span className="t5-chart-meta-name min-w-0 flex-1 truncate uppercase">{name}</span>
        <span className="t5-chart-meta-total shrink-0 tabular-nums">{meta.total}</span>
        <span
          className={`t5-chart-meta-overs shrink-0 ${
            tone === "bowler" ? "t5-chart-meta-overs--bowler" : ""
          }`}
        >
          <span className="t5-chart-mini-label block uppercase">OVERS</span>
          <span className="t5-chart-meta-overs-value tabular-nums">{meta.overs}</span>
        </span>
      </div>
      <div className="flex gap-[calc(12px*var(--t5-scale))]">
...
/** Worm / score comparison chart core (Theme 5). */
export default function WormChartCore({
  ...
  return (
    <ChartPageShell>
      ...
      <div className="t5-chart-body t5-chart-body-anim">
        <div className="flex min-h-0 flex-1 items-start">
          <div className="relative shrink-0">
            ...
            {markerLeft != null ? (
              <span
                className="t5-chart-xlabel absolute"
                style={{
                  left: `calc(${markerLeft}px * var(--t5-scale))`,
                  top: `calc((${CHART_PAD.T + plotH + 14}px) * var(--t5-scale))`,
                }}
              >
                {chart.markerOver}
              </span>
        ...
        <div className="flex shrink-0 gap-[calc(48px*var(--t5-scale))]">
