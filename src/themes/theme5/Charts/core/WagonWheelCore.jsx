...
const GRASS_GRADIENT_ID = "t5-wagon-wheel-grass";
...
function WagonWheelField({ shots, size = WHEEL_SIZE, animate = true }) {
  ...
  return (
    <svg
      width={`calc(${size}px * var(--t5-scale))`}
      height={`calc(${size}px * var(--t5-scale))`}
...
function WagonLegend() {
  return (
    <div className="t5-chart-wagon-legend">
      {LEGEND_ITEMS.map(([label, color]) => (
        <div key={label} className="flex items-center gap-[calc(8px*var(--t5-scale))]">
          <span
            className="t5-chart-legend-swatch shrink-0"
            style={{ background: color }}
          />
          <span className="t5-chart-legend whitespace-nowrap uppercase">{label}</span>
        </div>
      ))}
    </div>
  );
...
function WagonBigStat({ label, value, accent }) {
  return (
    <div className="t5-chart-wagon-stat">
      <div
        className="t5-chart-wagon-stat-accent"
        style={{ background: accent }}
        aria-hidden="true"
      />
      <div className="t5-chart-legend uppercase">{label}</div>
      <div
        className="t5-chart-big-stat mt-[calc(4px*var(--t5-scale))] tabular-nums leading-none"
        style={{ color: accent }}
      >
        {value}
      </div>
...
function WagonZoneBar({ label, value, total, color }) {
  ...
  return (
    <div className="min-w-0">
      <div className="mb-[calc(6px*var(--t5-scale))] flex items-baseline justify-between gap-[calc(12px*var(--t5-scale))]">
        <span className="t5-chart-legend shrink-0 whitespace-nowrap uppercase">
          {label}
        </span>
        <span className="t5-chart-legend-muted shrink-0 whitespace-nowrap">
          {value} runs · {pct}%
        </span>
      </div>
      <div className="t5-chart-zone-track">
        <div
          className="t5-chart-zone-fill motion-safe:animate-chart-grow-w motion-reduce:animate-none"
          style={{
            "--chart-bar-w": `${pct}%`,
            width: `${pct}%`,
...
function WagonStatsPanel({ player, stats }) {
  return (
    <div className="flex min-w-0 flex-col justify-center gap-[calc(22px*var(--t5-scale))]">
      <div className="min-w-0">
        <div className="t5-chart-legend t5-chart-legend-heading uppercase">SHOT MAP</div>
        <div className="t5-chart-player-name mt-[calc(8px*var(--t5-scale))] break-words uppercase leading-[1.05]">
          {player.name}
        </div>
        {player.teamName || player.role ? (
          <div className="t5-chart-legend-muted mt-[calc(6px*var(--t5-scale))]">
            {[player.teamName, player.role].filter(Boolean).join(" · ")}
          </div>
        ) : null}
      </div>

      <div className="grid grid-cols-3 gap-[calc(12px*var(--t5-scale))]">
...
/** Wagon wheel chart core (Theme 5). */
export default function WagonWheelCore({
  ...
  return (
    <ChartPageShell>
      ...
      <div className="t5-chart-wagon-panel t5-chart-body-anim motion-safe:animate-chart-fade-up motion-reduce:animate-none">
        <div
          className="t5-chart-wagon-grid"
          style={{
            gridTemplateColumns: `minmax(0, calc(${WHEEL_SIZE + 40}px * var(--t5-scale))) minmax(calc(280px * var(--t5-scale)), 1fr)`,
          }}
        >
          <div className="flex min-w-0 flex-col items-center justify-center gap-[calc(18px*var(--t5-scale))]">
