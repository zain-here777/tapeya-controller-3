...
const ANIMATION_DELAYS = ["0ms", "240ms", "480ms"];

/**
 * Shared action banner — three balanced labels with looping rise animation.
  ...
  return (
    <div className="controller-scaled relative w-full shrink-0 overflow-hidden font-controller select-none">
      <div className="relative flex h-bar w-full items-center overflow-hidden bg-panel-player">
        <div
          className="pointer-events-none absolute inset-0 animate-four-bar-pulse bg-[radial-gradient(ellipse_at_center,rgba(61,204,74,0.22)_0%,transparent_70%)]"
          aria-hidden="true"
        />

        <div className="relative z-[1] grid h-full w-full grid-cols-3 items-center">
          {labels.slice(0, 3).map((label, index) => (
            ...
            >
              <span
                className={`${labelClass} animate-four-rise inline-block text-center uppercase text-white`}
