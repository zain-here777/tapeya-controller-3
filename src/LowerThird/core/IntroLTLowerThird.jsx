...
/**
 * Intro lower third — matchup intro with tournament subtitle.
 * Uses MatchSummary shell, separators, and typography tokens.
 *
 * @param {Object} props
...
export default function IntroLTLowerThird({
  ...
  const { tournament = "LAHORE SUMMER CUP", teamA, teamB } = match;
    ...
    <div className="controller-scaled relative w-full shrink-0 overflow-hidden font-controller select-none">
      <div className="mb-[calc(45px*var(--cs))] w-full px-[calc(210px*var(--cs))]">
        <div className="match-summary-shell relative flex h-bar w-full items-stretch overflow-hidden">
          <IntroLogoSlot team={teamA} separator="right" />
          ...
          <IntroLogoSlot team={teamB} separator="left" />
        </div>
      </div>
    </div>
  );
...
function IntroLogoSlot({ team, separator }) {
  return (
