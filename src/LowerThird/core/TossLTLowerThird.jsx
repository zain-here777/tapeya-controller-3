...
/**
 * Toss lower third — matchup + toss result.
 * Same UI shell as IntroLT; content is toss-specific.
 *
 * @param {Object} props
 ...
 */
export default function TossLTLowerThird({
  match,
  baseConfig,
  ...
  const {
    tossResult = "ISLAMABAD UNITED WON THE TOSS AND ELECTED TO BAT FIRST",
    teamA,
    teamB,
  } = match;
    ...
    <div className="controller-scaled relative w-full shrink-0 overflow-hidden font-controller select-none">
      <div className="mb-[calc(45px*var(--cs))] w-full px-[calc(210px*var(--cs))]">
        <div className="match-summary-shell relative flex h-bar w-full items-stretch overflow-hidden">
          <TossLogoSlot team={teamA} separator="right" />
          ...
          <div className="relative z-[1] flex min-w-0 flex-1 flex-col bg-transparent">
            ...
            {tossResult ? (
              <div className="flex h-[calc(56px*var(--cs))] shrink-0 items-center justify-center border-t border-divider bg-transparent px-[calc(16px*var(--cs))]">
                <span className="ctrl-t-summary-title truncate uppercase text-white">
                  {tossResult}
                </span>
              </div>
            ) : null}
          ...
          <TossLogoSlot team={teamB} separator="left" />
        </div>
      </div>
    </div>
  );
...
function TossLogoSlot({ team, separator }) {
  return (
